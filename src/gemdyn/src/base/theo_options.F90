...
module theo_options
   ...
   !# * 'MTN_PINTYNL': nonlinear regime
      ...
      logical :: Theo_periodicX_L = .false.
contains
...
!
      ...
      integer :: ictr,kctr,ii
      real :: dx,dz,rad,inc,r
!     
!---------------------------------------------------------------------
!
...
!!$omp end parallel
      do i=1,Tr3d_ntr
         if (trim(Tr3d_name_S(i))=='BLOB') then
            ...
            do k=1,G_nk
            do ii=1,l_ni
               r = sqrt( ( (dble(ii-ictr)* dble(dx)) **2 + ( (dble(k)-kctr)* dble(dz)) **2) )
               inc = exp( - (r/100.d0)**2 )
               tracers_P(i)%pntr(ii,j,k) = 22.* inc
            end do
            end do
         endif
      end do
