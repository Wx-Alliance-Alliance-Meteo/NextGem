...
!**s/r set_world_view
      ...
      integer, external :: adv_config, gemdm_config, set_io_pes,&
                           domain_decomp,get_file_unit,sol_decomp

      character(len=50) :: LADATE
      integer :: istat,options,wload,hzd,monot,massc,unf
      integer :: f1,f2,f3,f4,myproc,numproc,couleur
...
!     Read namelists from file Path_nml_S
      ...
      call cp_n_open (unf,COMM_GRID,trim( Path_nml_S ))
      if (unf > 0) then
...
! Initialize PEs for parallel output
