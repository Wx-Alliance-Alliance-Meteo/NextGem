...
!** fgmres - Flexible generalized minimum residual method(with restarts).
      ...
      subroutine sol_fgmres_QUAD ( F_print_L )
      use ISO_C_BINDING
      ...
      integer :: HLT_np, HLT_start, HLT_end, k0,kn
...
!
      ...
      do                        ! MAIN OUTER ITERATION
         ...
         do initer=1,sol_im
            ...
            Cpntr= c_loc( vv(Sol_imin,Sol_jmin,1,initer) )
            ...
            enddo
            call MPI_allreduce(v_avg_8(1:initer+1,:), v_prod(1:initer+1,:), (initer+1)*2, MPI_double_precision, MPI_sum, COMM_MULTIGRID, ierr)
            do it=1,initer-1
            ...
            if (  almost_zero( rr(initer+1,initer+1) ) ) then
               if (ptopo_myproc==0) print*,'========= FGMRES Happy breakdown !! ===='
               exit
            endif
