...
! Create space for diagnostic level values
      ...
      allocate (pw_uu_plus0(1:l_ni,1:l_nj,1:l_nk))
      allocate (pw_vv_plus0(1:l_ni,1:l_nj,1:l_nk))
      allocate (pw_tt_plus0(1:l_ni,1:l_nj,1:l_nk))
      allocate (qw_phy     (1:l_ni,1:l_nj,1:l_nk))
      allocate (qw_dyn     (1:l_ni,1:l_nj,1:l_nk))
      allocate (tdu(l_minx:l_maxx,l_miny:l_maxy,1:l_nk))
      allocate (tdv(l_minx:l_maxx,l_miny:l_maxy,1:l_nk))
      allocate (tv (l_minx:l_maxx,l_miny:l_maxy,1:l_nk))
      allocate (qt1i(1:l_ni,1:l_nj,1:l_nk+1))
      allocate (delq(1:l_ni,1:l_nj))

      allocate (pm_dyn_8(1:l_ni,1:l_nj,1:l_nk+1))
      allocate (p0_8    (1:l_ni,1:l_nj))
...
! We put mandatory variables in the WhiteBoard
      ...
      err= wb_put('itf_phy/TLIFT'       , tlift        , WB_REWRITE_AT_RESTART)
      err= wb_put('itf_phy/DYNOUT'      , Out3_accavg_L, WB_REWRITE_AT_RESTART)
      err= wb_put('itf_phy/slt_winds'   , Adz_slt_winds, WB_REWRITE_AT_RESTART)

! Complete physics initialization (see phy_init for interface content)
            
      Phy_npes = Inp_npes ! eventually a user choice independant fron the dynamics Inp_npes
      err= io_pes (Phy_comm_id,Phy_npes)
...
!
contains
      integer function io_pes ( F_comm_id,F_npes )
      use iso_c_binding
      use glb_ld
      use domains
      use ptopo
