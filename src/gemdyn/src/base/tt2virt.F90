...
!**   s/r tt2virt - Compute virtual temperature in pw_tt_plus using F_t when F_tt2tv=.t.
!                   Compute true temperature in F_t using tt1 when F_tt2tv=.f.
...
!!$omp parallel
      call sumhydro (sumq_8, Minx, Maxx, Miny, Maxy, Nk, Tr3d_ntr, trt1, Schm_wload_L)
!!$omp end parallel

      if (.not.F_tt2tv) then
         call mfottvh2 (F_t, tvirt, tracers_P(Tr3d_hu)%pntr, real(sumq_8),&
                        minx, maxx, miny, maxy, l_nk, &
                        1-G_halox, l_ni+G_halox, 1-G_haloy, l_nj+G_haloy, F_tt2tv)
      else
         call mfottvh2 (tt, F_t, tracers_P(Tr3d_hu)%pntr, real(sumq_8),&
                        minx, maxx, miny, maxy, l_nk, &
                        1-G_halox, l_ni+G_halox, 1-G_haloy, l_nj+G_haloy, F_tt2tv)
      end if
!
...
!
      return
      end subroutine tt2virt
      
      subroutine tt2virt_omp (F_t, F_tt2tv, Minx, Maxx, Miny, Maxy, Nk)
      use dyn_fisl_options
      use glb_ld
      use gem_options
      use gmm_pw
