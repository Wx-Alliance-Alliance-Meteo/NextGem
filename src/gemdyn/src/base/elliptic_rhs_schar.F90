...
!**s/r elliptic_rhs_schar - Compute right hand side of the elliptic problem
      ...
      subroutine elliptic_rhs_schar ( F_dt_8, k0, k0t )
      ...
      integer :: km,i00,inn,j00,jnn,dim,ub,ni,nj
...
! this exception at k=1 will be removed soon and km should be replaced with simply k-1
          ...
          !  if ((i==217).and.(j==15)) print*, 'haha2: ',Sol_rhs(i,j,k),ubx*GVM%mc_Ix_8(i,j,k) + vby*GVM%mc_Iy_8(i,j,k)
            endif
         end do
         end do
      end do
...
! Specific scope is here important for wka,wkb
      ...
      call statf_dm (Sol_rhs, 'RHS', 1, 'ell1', 1,ni,1,nj,1,l_nk,1+Glb_pil_w,1+Glb_pil_s,1,G_ni-Glb_pil_e,G_nj-Glb_pil_n,l_nk,8)
!      print*, Sol_rhs(415,15,65),Sol_rhs(217,15,1),l_nj/2+1
!     call gem_stop
!
!     ---------------------------------------------------------------
!
      return
      end subroutine elliptic_rhs_schar
