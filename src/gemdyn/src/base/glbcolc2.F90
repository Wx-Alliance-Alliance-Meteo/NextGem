...
!       Receive the local data (LD) segments from all other processors
         ...
         do iproc = 1, Ptopo_numproc-1
            call MPI_recv ( len, 5, MPI_INTEGER, iproc, &
                            tag, COMM_GRID, status, err )
            if (len > 0) then
               call MPI_recv ( buf, len, MPI_REAL, iproc, &
                               tag,COMM_GRID, status, err )
               len = 0
               do k = g_kd, g_kf
...
!       Send local data (LD) segment to processor 1
         ...
         call MPI_send ( len, 5  , MPI_INTEGER, 0, tag, COMM_GRID,err )
         if (len > 0) &
         call MPI_send ( buf, len, MPI_REAL   , 0, tag, COMM_GRID,err )
