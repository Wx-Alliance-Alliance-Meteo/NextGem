...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   integer function exchange_field(F_fld) result(F_status)
      ...
      if (Grd_yinyang_L) then
         call yyg_xchng (F_fld, l_minx,l_maxx,l_miny,l_maxy,l_ni,l_nj,&
                         1,.false.,'NEAREST',.true.)
       !  call yyg_int_xch_scal(F_fld,1,.false.,'NEAREST',.true.)
      else
         call gem_xch_halo (F_fld,l_minx,l_maxx,l_miny,l_maxy,1,-1)
      end if
      ! Successful completion
      F_status = CLDOBJ_OK
