...
!**s/r out_tracer - output tracer
      ...
      integer i,j,k,ii,n,nko,model_nk,knd,istat,indxtr,ind0(1)
...
!        Retreieve vertical coordinate description
         ...
         hybt(G_nk+1) = hybt(G_nk+2) !diag level (ignore surface level)
         ind0(1)=G_nk+1
...
!                                      F_start=lijk, F_end=uijk, F_quiet=.true.)
               ...
               if (model_nk > G_nk) then
                  Out_stag_S(3:3)= 'D'
                  call out_fstecr ( w4,l_minx,l_maxx,l_miny,l_maxy,hybt,&
                               Outd_var_S(ii,set),Outd_convmult(ii,set),&
                               Outd_convadd(ii,set),Level_kind_diag,-1 ,&
                               G_nk+1,ind0,1,Outd_nbit(ii,set),.false. )
                  Out_stag_S(3:3)= ' '
               endif
