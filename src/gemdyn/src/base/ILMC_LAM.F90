...
!**s/r ILMC_LAM - Ensures monotonicity of interpolated field while preserving mass (Sorenson et al.,2013)
      ...
      integer :: i,j,k,sweep,i_rd,j_rd,k_rd,ii,ix,jx,kx,kx_m,kx_p,j1,j2, &
                 ...
                 g_i0,g_in,g_j0,g_jn,il_,ir_,jl_,jr_,il_cor,ir_cor,jl_cor,jr_cor,&
                 err,hg(4),hg_cor(4)
...
!      call gtmg_start (73, 'ILMC_', 38)
      ...
      hg(1) = il -il_ ; hg(2) = ir_-ir ; hg(3) = jl -jl_ ; hg(3) = jr_-jr

      !Print if Halo needs to be reduced for PE close to W/E/S/N LAM boundary 
      !----------------------------------------------------------------------
      if (.not.done_print_L) then

         hg(1) = il -il_ ; hg(2) = ir_-ir ; hg(3) = jl -jl_ ; hg(4) = jr_-jr
         call MPI_allreduce (hg,hg_cor,4,MPI_INTEGER,MPI_MAX,COMM_GRID,err)
         il_cor= hg(1) ; ir_cor= hg(2) ; jl_cor= hg(3) ; jr_cor = hg(4)
               ...
               do sweep = 1,Adz_ILMC_sweep_max
                  ...
                  do jx = j1,j2
                     ...
                     do kx = kx_m,kx_p
                        ...
                        limit_i_L = limit_i_L.and.(kx>k-sweep.and.kx<k+sweep)
...
!      call gtmg_start (94, 'X_HALO', 73)
      ...
      call gem_xch_halo_st ( air_mass_m        ,l_minx,l_maxx,l_miny,l_maxy,l_nk,-1) 
      call gem_xch_halo_st ( F_ilmc            ,l_minx,l_maxx,l_miny,l_maxy,l_nk,-1) 
      call gem_xch_halo_st ( Adz_post(F_ns)%min,l_minx,l_maxx,l_miny,l_maxy,l_nk,-1) 
      call gem_xch_halo_st ( Adz_post(F_ns)%max,l_minx,l_maxx,l_miny,l_maxy,l_nk,-1) 

! soon when this routine is HLT compliant
!!$      call HLT_split (1, l_nk, local_np, HLT_start, HLT_end)
!!$      if (Adz_set_post_tr==1) &
!!$      call gem_xch_halo ( air_mass_m(l_minx,l_miny,HLT_start),&
!!$                          l_minx,l_maxx,l_miny,l_maxy,local_np,-1) 
!!$      call gem_xch_halo ( F_ilmc(l_minx,l_miny,HLT_start),&
!!$                          l_minx,l_maxx,l_miny,l_maxy,local_np,-1) 
!!$      call gem_xch_halo ( Adz_post(F_ns)%min(l_minx,l_miny,HLT_start),&
!!$                          l_minx,l_maxx,l_miny,l_maxy,local_np,-1) 
!!$      call gem_xch_halo ( Adz_post(F_ns)%max(l_minx,l_miny,HLT_start),&
!!$                          l_minx,l_maxx,l_miny,l_maxy,local_np,-1) 
...
!**s/r ILMC_LAM_write - Write ILMC_LAM diagnostics based on F_numero if verbose is activated
      ...
      integer :: err,iprod,l_reset(5),g_reset(5),comm
...
!
      ...
      elseif (F_numero==3) then
         ...
         !--------------------------
         comm = COMM_grid
         if (Grd_yinyang_L) comm= COMM_multigrid
         ...
         call MPI_allreduce (l_reset,g_reset,5,MPI_INTEGER,MPI_SUM,comm,err)
