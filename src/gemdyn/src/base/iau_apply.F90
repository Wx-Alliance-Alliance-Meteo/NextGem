...
!!$   call msg(MSG_INFO,'IAU YES/NO?: '//trim(msg_S))
   ...
   IF_INIT: if (.not.is_init_L) then
      ...
      istat = ptopo_io_set(Phy_npes)
      istat = inputio_new(inputobj, jdateo, nint(Cstv_dt_8), " ", &
           Path_input_S, Grd_global_gid, Grd_glbcore_gid, Phy_comm_id, &
...
! The need for the following needs to be established before
! re-implementation in the context of HLT
...
!!$               call gem_xch_halo (data0,lijk(1),uijk(1),lijk(2),uijk(2),uijk(3),-1)
!!$               call gem_xch_halo (data1,lijk(1),uijk(1),lijk(2),uijk(2),uijk(3),-1)
!!$            else
...
!!$            print '(a,i4," : ",f10.5," : ",f10.5," < ",f10.5)','[iau_apply] ', F_kount, data0(l_ni/2, l_nj/2, l_nk/2), minval(data0), maxval(data0)
         ...
         if (associated(myptr0)) then
            if (iname0_S == 'p0') then
               ...
               call tt2tvirt (vt, pw_tt_plus, 1,l_ni, 1,l_nj)
