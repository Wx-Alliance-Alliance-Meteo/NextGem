...
!DIR$ SIMD
            do i= i0, in
               ...
               s2(i)= GVM%mc_iJz_8(i+1,j,k-1)*(ext_q(i+1,j,k  )-ext_q(i+1,j,k-1)) !dqdz(i+1,k-1)
               s3(i)= GVM%mc_iJz_8(i-1,j,k  )*(ext_q(i-1,j,k+1)-ext_q(i-1,j,k  )) !dqdz(i-1,k  )
               s4(i)= GVM%mc_iJz_8(i-1,j,k-1)*(ext_q(i-1,j,k  )-ext_q(i-1,j,k-1)) !dqdz(i-1,k-1)
               s5(i)= GVM%mc_iJz_8(i,j+1,k  )*(ext_q(i,j+1,k+1)-ext_q(i,j+1,k  )) !dqdz(j+1,k  )
               s6(i)= GVM%mc_iJz_8(i,j+1,k-1)*(ext_q(i,j+1,k  )-ext_q(i,j+1,k-1)) !dqdz(j+1,k-1)
               s7(i)= GVM%mc_iJz_8(i,j-1,k  )*(ext_q(i,j-1,k+1)-ext_q(i,j-1,k  )) !dqdz(j-1,k  )
               s8(i)= GVM%mc_iJz_8(i,j-1,k-1)*(ext_q(i,j-1,k  )-ext_q(i,j-1,k-1)) !dqdz(j-1,k-1)
               ...
               w4(i)= (ext_q(i,j  ,k)-ext_q(i,j-1,k))*geomh_invDYM_8(j) !dqdy(j-1)
