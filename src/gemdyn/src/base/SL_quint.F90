...
!**s/r SL_quint
      ...
      subroutine SL_quint ( F_qut, F_lin , F_min , F_max , F_in, F_xyz, &
      ...
      real    :: xyz_1,xyz_2
      real(kind=REAL64), parameter :: ov120 = 1.d0/120.d0, ov24 = 1.d0/24.d0, ov12 = 1.d0/12.d0
      ...
      real(kind=REAL64) :: px(0:5),py(0:5),qz(6),del,del123,del210,&
...
!
      ...
      do n=1,F_num
         ...
         ii1   = xyz_1
         del   = xyz_1 - dble(ii1)
         del123= (del-1.d0)*(del-2.d0)*(del-3.d0)
         del210= (del+2.d0)*(del+1.d0)* del
         px(0) =-(del+1.d0)* del   * del123*ov120
         px(1) = (del+2.d0)* del   * del123*ov24
         px(2) =-(del+2.d0)*(del+1.d0)* del123*ov12
         px(3) =  del210*(del-2.d0)*(del-3.d0)*ov12
         px(4) =- del210*(del-1.d0)*(del-3.d0)*ov24
         px(5) =  del210*(del-1.d0)*(del-2.d0)*ov120
         ...
         del123= (del-1.d0)*(del-2.d0)*(del-3.d0)
         del210= (del+2.d0)*(del+1.d0)* del
         py(0) =-(del+1.d0)* del   * del123*ov120
         py(1) = (del+2.d0)* del   * del123*ov24
         py(2) =-(del+2.d0)*(del+1.d0)* del123*ov12
         py(3) =  del210*(del-2.d0)*(del-3.d0)*ov12
         py(4) =- del210*(del-1.d0)*(del-3.d0)*ov24
         py(5) =  del210*(del-1.d0)*(del-2.d0)*ov120
         ...
         if (.not.zqutic_L.and..not.zcubic_L) kk1 = min(F_kn-1,max(F_k0,kk1))
         ...
         do i=1,F_nptr
            call biquiHQV_lin_min_max_zyx (&
               F_qut(n,i),F_lin(n,i),F_min(n,i),F_max(n,i),F_in(o1,i),&
               px,py,qz,lx,ly,lz,nit,nij,zqutic_L,zcubic_L)
         end do
