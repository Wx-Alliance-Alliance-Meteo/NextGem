...
#include <rmnlib_basics.hf>
   ...
   public
   interface inp_src_vert3d
     module procedure inp_src_vert3d_gz3d
     module procedure inp_src_vert3d_vgd
   end interface
...
!
      ...
      call gtmg_start (Inp_gtmg(1)+2, 'Vert_int', Inp_gtmg(2))
...
!                    interpolation to F_nd Arakawa grid destinations
      ...
      integer i, j, k, idst, err, nz, n1,n2,n3, nrec, liste(nlis),&
              liste_sorted(nlis),lislon,maxdim_wk2,n
      integer ni_dest,nj_dest,deb
...
!        lislon should be >0 here so, nz  >=1
         ...
         if (local_nk > 0) then
            ...
            write(output_unit,1001) 'Interpolating: ',trim(F_var_S),&
                           trim(nomvar),', nka= ',lislon,',valid: ',&
                           Inp_datev,' on ',F_hgrid_S(idst),' grid'
         end if
         ...
         if (err == 2) then
            write(output_unit,1001) 'EXTRApolating: ',trim(F_var_S),&
                trim(nomvar),', nka= ', lislon,',valid: ',Inp_datev,&
                ' on ',F_hgrid_S(idst),' grid'
         end if
         err = ezsetopt ( 'USE_1SUBGRID', 'NO' )
 ...
 999  call MPI_bcast ( lislon, 2, MPI_INTEGER, Inp_bcast, &
                       COMM_grid, err ) ! NOTE: this is bcasting lislon AND nz
      F_nka= lislon
      ...
      if (F_nka > 0) then
         ...
         inp_read_mt= 0
         if (F_nka >= 1) then
            if (Inp_iope < 0) allocate ( F_ip1(F_nka) )
            call MPI_bcast ( F_ip1, F_nka, MPI_INTEGER, &
                             Inp_bcast, COMM_grid, err )
         end if
       ...
       !  call glbstat (F_dest,nomvar,'',l_minx,l_maxx,l_miny,l_maxy,1,F_nka,1,G_ni,1,G_nj,1,F_nka)
        ! print*, F_dest(l_minx,l_miny,1),F_dest(l_ni+G_halox,l_nj+G_haloy,F_nka)
      else

         inp_read_mt= -1
         if ((Inp_iope >= 0).and.(.not.quiet_L)) &
         write(output_unit,'(7a)') ' FIELD: ',trim(F_var_S),':',&
         trim(nomvar),' valid: ',Inp_datev, 'NOT FOUND'

      end if
 876  call gtmg_stop (Inp_gtmg(1))
...
!
      ...
      Nest_current_dayfrac_8= diffd*86400.d0
...
!
      ...
      if (associated(wrk)) then
         ...
         if (Inp_kind == 2.or.Dynamics_sw_L) then
            ...
            if (nk == F_nka) then
               ...
               call gem_xch_halo_st (F_su, l_minx, l_maxx, l_miny, l_maxy, 1, -1)
               call gem_xch_halo_st (F_sv, l_minx, l_maxx, l_miny, l_maxy, 1, -1)
            else
               err = -1
            end if
         end if
...
!
      ...
      call gem_xch_halo_st (F_u, l_minx, l_maxx, l_miny, l_maxy, 1, -1)
      call gem_xch_halo_st (F_v, l_minx, l_maxx, l_miny, l_maxy, 1, -1)

      if ( F_sleve_L ) then
         allocate (F_lsq(l_minx:l_maxx,l_miny:l_maxy), &
                   F_lsu(l_minx:l_maxx,l_miny:l_maxy), &
                   F_lsv(l_minx:l_maxx,l_miny:l_maxy) )
         F_lsq(1-G_halox:l_ni+G_halox,1-G_haloy:l_nj+G_haloy) = &
         F_orols(1-G_halox:l_ni+G_halox,1-G_haloy:l_nj+G_haloy) / grav_8
         do j=1-G_haloy,l_nj+G_haloy
            do i=1-G_halox,l_ni+G_halox-1
         ...
         call gem_xch_halo_st (F_lsu, l_minx, l_maxx, l_miny, l_maxy, 1, -1)
         call gem_xch_halo_st (F_lsv, l_minx, l_maxx, l_miny, l_maxy, 1, -1)
      end if
!
!---------------------------------------------------------------------
!
...
!
 ...
 999  call MPI_bcast (lislon, 3, MPI_INTEGER, Inp_bcast, &
                      COMM_grid, err)
      F_nka= lislon
      ...
      if (F_nka > 0) then
         ...
         if (Inp_iope >= 0) write(output_unit,'(3a)') &
...
!
      ...
      call gem_error(istat,'inp_3dpres','Error computing pressure')
!
!---------------------------------------------------------------------
!
      return
...
!
      F_nk= ubound(F_ip1,1)
      if (Inp_src_hauteur_L.and..not.Dynamics_sw_L) then
...
!---------------------------------------------------------------------
!
      call inp_3dhgts ( F_vgd, F_ip1, F_sfc, F_sfcL, F_dest, 1, F_nk)
!
!---------------------------------------------------------------------
!
      return
...
!     local variables
      logical :: urt1_L
      integer :: i,j,k,n,cnt,err,nka,dim,deb
      ...
      if (INs_server_L) then
         ...
         do n=1,INs_recv_nreqs
            if ((trim(SRL(n)%vname(2)) == 'UVRT1' ) .or. &
               ...
               urt1_L= .true.
               if (lun_out>0) write(6,'(a,a15,a,i4)') ' I-svr FOUND: ','UV',&
                                             ' at indexe: ',n
            endif
         end do
...
!
      ...
      logical function inp_src_vert3d_gz3d (F_var_S, F_3dv)
      implicit none
...
!
      ...
      if (associated(F_3dv%sfc )) deallocate (F_3dv%sfc )
      if (associated(F_3dv%ip1 )) deallocate (F_3dv%ip1 )
      nullify (F_3dv%valq,F_3dv%valu,F_3dv%valv,F_3dv%sfc,F_3dv%ip1,val)
      F_3dv%me_L = .false. ; F_3dv%mels_L = .false. ; F_3dv%sleve_L = .false.

      grid_S=['Q','U','V']
      allocate (F_3dv%sfc (l_minx:l_maxx,l_miny:l_maxy,2))

      err_mt = inp_read_mt (F_var_S,grid_S, val,3,&
                F_3dv%ip1, lislon,F_quiet_L=.true.)
      inp_src_vert3d_gz3d= ((err_mt == 0) .and. (lislon>1))

      if (inp_src_vert3d_gz3d) then
         F_3dv%nk= lislon
         ...
         F_3dv%me_L = .true.
         F_3dv%sfc(:,:,1) = val(l_minx:l_maxx,l_miny:l_maxy,F_3dv%nk)
      ...
      nullify (val,ip1_list)

      err_mt = inp_read_mt ('MELS','Q', val,1,&
                ip1_list, lislon)!,F_quiet_L=.true.)      
      if ((err_mt == 0) .and. (lislon>0)) then
         F_3dv%mels_L = .true.
         F_3dv%sfc(:,:,2) = val(l_minx:l_maxx,l_miny:l_maxy,1)
         deallocate (val,ip1_list)
      else
         F_3dv%sfc(:,:,2) = 0.
      endif        
    
 9000 format(1x,'INP_SRC_VERT3D_GZ3D: ', i3,' levels of ',a,&
                        ' found in input file with kind=',i5)
!
!---------------------------------------------------------------------
!
       return
       end function inp_src_vert3d_gz3d

      logical function inp_src_vert3d_vgd (F_vgd, F_3dv)
      implicit none
      ...
      logical :: sleve_L
      character(len=1) :: grid_S(3)
      integer :: i,j,k, err(6), err_me, err_mels, src_nka, cnt
      integer, dimension (:), pointer :: ip1_list, ip1_m, ip1_t
      real :: src_rcoef(4)
      real, dimension (:,:,:), pointer :: src_me, src_mels
      real(kind=REAL64), dimension(:), pointer :: src_hybmA,src_hybtA,src_hybmB,src_hybtB,src_hybmC,src_hybtC,wkpt8
!
!---------------------------------------------------------------------
!
      inp_src_vert3d_vgd = .false.

      F_3dv%nk= -1 ; F_3dv%kind= 21
      if (associated(F_3dv%valq)) deallocate (F_3dv%valq)
      if (associated(F_3dv%valu)) deallocate (F_3dv%valu)
      if (associated(F_3dv%valv)) deallocate (F_3dv%valv)
      if (associated(F_3dv%sfc )) deallocate (F_3dv%sfc )
      if (associated(F_3dv%ip1 )) deallocate (F_3dv%ip1 )
      nullify (F_3dv%valq,F_3dv%valu,F_3dv%valv,F_3dv%sfc,F_3dv%ip1,ip1_m,ip1_t)
      F_3dv%me_L = .false. ; F_3dv%mels_L = .false. ; F_3dv%sleve_L = .false.

      grid_S=['Q','U','V']

      src_rcoef = -1. ; err= -1
      err(1) = vgd_get ( F_vgd, key='RC_1 - first R-coef value' ,value=src_rcoef(1))
      err(2) = vgd_get ( F_vgd, key='RC_2 - second R-coef value',value=src_rcoef(2))
      err(3) = vgd_get ( F_vgd, key='RC_3 - third R-coef value' ,value=src_rcoef(3))
      err(4) = vgd_get ( F_vgd, key='RC_4 - forth R-coef value' ,value=src_rcoef(4))
      err(5) = vgd_get ( F_vgd, key='VIPM - level ip1 list (m)' ,value=ip1_m)
      err(6) = vgd_get ( F_vgd, key='VIPT - level ip1 list (t)' ,value=ip1_t)
      if ( minval(err) < 0) return
      sleve_L = minval(src_rcoef(3:4)) >=0.
      nullify (src_me, src_mels, ip1_list)
      err(1)= inp_read_mt ( 'ME'  , grid_S, src_me  , 3, ip1_list, src_nka )
      if ( err(1) < 0 ) return
      deallocate (ip1_list) ; nullify (ip1_list)
      F_3dv%me_L = .true.
      if (sleve_L) then
         err(1)= inp_read_mt ( 'MELS', grid_S, src_mels, 3, ip1_list, src_nka )
         if ( err(1) < 0) return
         deallocate (ip1_list) ; nullify (ip1_list)
         F_3dv%mels_L = .true.
      else
         allocate (src_mels(lbound(src_me,1):ubound(src_me,1),lbound(src_me,2):ubound(src_me,2),3))
         src_mels= 0.
      endif
      err(1) = vgd_get ( F_vgd, key='NL_M - number of momentum levels' ,value=src_nka)
      if ( err(1) < 0 ) return
      allocate ( src_hybmA(src_nka), src_hybtA(src_nka),&
                 src_hybmB(src_nka), src_hybtB(src_nka),&
                 src_hybmC(src_nka), src_hybtC(src_nka))
      nullify(wkpt8)
      err(1)= vgd_get ( F_vgd, key='CA_M - vertical A coefficient (m)' ,value=wkpt8)
      if (err(1)==0)  src_hybmA= wkpt8(1:src_nka); deallocate(wkpt8); nullify(wkpt8)
      err(2)= vgd_get ( F_vgd, key='CB_M - vertical B coefficient (m)' ,value=wkpt8)
      if (err(2)==0)  src_hybmB= wkpt8(1:src_nka); deallocate(wkpt8); nullify(wkpt8)
      err(3)= vgd_get ( F_vgd, key='CC_M - vertical C coefficient (m)' ,value=wkpt8)
      if (err(3)==0)  src_hybmC= wkpt8(1:src_nka); deallocate(wkpt8); nullify(wkpt8)
      err(4)= vgd_get ( F_vgd, key='CA_T - vertical A coefficient (t)' ,value=wkpt8)
      if (err(4)==0)  src_hybtA= wkpt8(1:src_nka); deallocate(wkpt8); nullify(wkpt8)
      err(5)= vgd_get ( F_vgd, key='CB_T - vertical B coefficient (t)' ,value=wkpt8)
      if (err(5)==0)  src_hybtB= wkpt8(1:src_nka); deallocate(wkpt8); nullify(wkpt8)
      err(6)= vgd_get ( F_vgd, key='CC_T - vertical C coefficient (t)' ,value=wkpt8)
      if (err(6)==0)  src_hybtC= wkpt8(1:src_nka); deallocate(wkpt8); nullify(wkpt8)
      if ( minval(err(1:6) ) < 0) return
      src_nka= src_nka-2
      cnt=0
      allocate (F_3dv%valq(lbound(src_me,1):ubound(src_me,1),lbound(src_me,2):ubound(src_me,2),2*src_nka+1),&
                F_3dv%valu(lbound(src_me,1):ubound(src_me,1),lbound(src_me,2):ubound(src_me,2),2*src_nka+1),&
                F_3dv%valv(lbound(src_me,1):ubound(src_me,1),lbound(src_me,2):ubound(src_me,2),2*src_nka+1),&
                F_3dv%sfc (lbound(src_me,1):ubound(src_me,1),lbound(src_me,2):ubound(src_me,2),2)          ,&
                F_3dv%ip1(2*src_nka+1) )
      F_3dv%sfc(:,:,1) = src_me  (:,:,1)
      F_3dv%sfc(:,:,2) = src_mels(:,:,1)
      do k=1,src_nka
         cnt= cnt+1
         F_3dv%ip1(cnt  ) = ip1_m(k)
         F_3dv%ip1(cnt+1) = ip1_t(k)
         do j=lbound(src_me,2), ubound(src_me,2)
            do i=lbound(src_me,1), ubound(src_me,1)
               F_3dv%valq(i,j,cnt  )= src_hybmA(k)+ (src_hybmB(k)*src_me(i,j,1)+src_hybmC(k)*src_mels(i,j,1))
               F_3dv%valq(i,j,cnt+1)= src_hybtA(k)+ (src_hybtB(k)*src_me(i,j,1)+src_hybtC(k)*src_mels(i,j,1))
               F_3dv%valu(i,j,cnt  )= src_hybmA(k)+ (src_hybmB(k)*src_me(i,j,2)+src_hybmC(k)*src_mels(i,j,2))
               F_3dv%valu(i,j,cnt+1)= src_hybtA(k)+ (src_hybtB(k)*src_me(i,j,2)+src_hybtC(k)*src_mels(i,j,2))
               F_3dv%valv(i,j,cnt  )= src_hybmA(k)+ (src_hybmB(k)*src_me(i,j,3)+src_hybmC(k)*src_mels(i,j,3))
               F_3dv%valv(i,j,cnt+1)= src_hybtA(k)+ (src_hybtB(k)*src_me(i,j,3)+src_hybtC(k)*src_mels(i,j,3))
            end do
         end do
         cnt= cnt+1
      end do
      do j=lbound(src_me,2), ubound(src_me,2)
         do i=lbound(src_me,1), ubound(src_me,1)
            F_3dv%valq(i,j,2*src_nka+1)= src_me(i,j,1)
            F_3dv%valu(i,j,2*src_nka+1)= src_me(i,j,2)
            F_3dv%valv(i,j,2*src_nka+1)= src_me(i,j,3)
         end do
      end do
      F_3dv%ip1(2*src_nka+1) = ip1_m(src_nka+1)
      F_3dv%nk= src_nka+1
      deallocate (src_hybmA,src_hybtA,src_hybmB,src_hybtB,src_hybmC,src_hybtC,src_me,src_mels,ip1_m,ip1_t)
      inp_src_vert3d_vgd = .true.
      if (Lun_out > 0) write(lun_out,9000) F_3dv%nk,'VGD',F_3dv%kind

 9000 format(1x,'INP_SRC_VERT3D_VGD: ', i3,' levels of ',a,&
                       ' found in input file with kind=',i5)
!
!---------------------------------------------------------------------
!
       return
       end function inp_src_vert3d_vgd
