...
!** delQ - Precompute Q hor. and vert. derivatives
      ...
      subroutine delQ5th ( F_q, F_minx,F_maxx,F_miny,F_maxy, &
...
!     ---------------------------------------------------------------
!
!      call delQ ( F_q, F_minx,F_maxx,F_miny,F_maxy, &
!                  F_Qu,F_Qv,F_Qw, F_Qq, F_k0,F_kn )
!      return
      nk= F_kn-F_k0+1
      ...
      do k=0,G_nk
         ...
         do j= 1,l_nj
            do i= 1,l_ni
               do ii=-2,3
                  dqx(ii) = F_q(i+ii,j,km2) * QDm2t(1,k) & 
                          + F_q(i+ii,j,km1) * QDm2t(2,k) & 
                          + F_q(i+ii,j,k  ) * QDm2t(3,k) & 
                          + F_q(i+ii,j,kp1) * QDm2t(4,k) & 
                          + F_q(i+ii,j,kp2) * QDm2t(5,k) & 
                          + F_q(i+ii,j,kp3) * QDm2t(6,k)

                  dqy(ii) = F_q(i,j+ii,km2) * QDm2t(1,k) & 
                          + F_q(i,j+ii,km1) * QDm2t(2,k) & 
                          + F_q(i,j+ii,k  ) * QDm2t(3,k) & 
                          + F_q(i,j+ii,kp1) * QDm2t(4,k) & 
                          + F_q(i,j+ii,kp2) * QDm2t(5,k) & 
                          + F_q(i,j+ii,kp3) * QDm2t(6,k)
               end do
      ...
      do k=1,G_nk
         ...
         do j= 1, l_nj
            do i= 1, l_ni
               u = dqdzx(i,j,km3) * QWt2m(1,k) &
                 + dqdzx(i,j,km2) * QWt2m(2,k) &  
                 + dqdzx(i,j,km1) * QWt2m(3,k) &  
                 + dqdzx(i,j,k  ) * QWt2m(4,k) &  
                 + dqdzx(i,j,kp1) * QWt2m(5,k) &  
                 + dqdzx(i,j,kp2) * QWt2m(6,k)  
               v = dqdzy(i,j,km3) * QWt2m(1,k) &
                 + dqdzy(i,j,km2) * QWt2m(2,k) &  
                 + dqdzy(i,j,km1) * QWt2m(3,k) &  
                 + dqdzy(i,j,k  ) * QWt2m(4,k) &  
                 + dqdzy(i,j,kp1) * QWt2m(5,k) &  
                 + dqdzy(i,j,kp2) * QWt2m(6,k)
               !--- remains second order for now ---
               F_Qq(i,j,k)= GVM%mc_iJz_8(i,j,k)*(F_q(i,j,k+1)-F_q(i,j,k))
               qbz        = half*(F_q(i,j,k)+F_q(i,j,k+1))
!!$               qbz = F_q(i,j,km2) * QWm2t(1,k) &
!!$                   + F_q(i,j,km1) * QWm2t(2,k) &
!!$                   + F_q(i,j,k  ) * QWm2t(3,k) &
!!$                   + F_q(i,j,kp1) * QWm2t(4,k) &
!!$                   + F_q(i,j,kp2) * QWm2t(5,k) &
!!$                   + F_q(i,j,kp3) * QWm2t(6,k)
