...
!
      ...
      if (Spn_freq < 0.0 )  then
              write(Lun_out,1002)
              stop
      endif

      ! Apply spectral nudging every Spn_interval steps, beginning with the third model step
      if (( Lctl_step > 2 ) .and. ( mod(Lctl_step,Spn_interval) == 0 ))  then 
         ...
         if (Spn_weight_L) then
            ! Adjust the nudging weight to have more strength near the valid-times of the
            ! driving fields
 ...
 1002 format(/' SPN_MAIN, Spn_freq must be specified !!!'/)
!
!----------------------------------------------------------------------
!
      return
