...
!
!!$omp parallel private (local_np, HLT_start, HLT_end,nu_dif,lnr,pwr,nn,mm) shared (wk1)
      ...
      if (nu_dif < 1.0e-10) goto 987

      nn = pwr/2

      call gem_xch_halo (F_f2dif(Minx,Miny,HLT_start),Minx,Maxx,Miny,Maxy,local_np,-1)
      ...
      do mm=1,nn
         ...
         call theo_hdif(F_f2dif, wk1, l_minx,l_maxx,l_miny,l_maxy,&
                                                   Nk, nu_dif, mm,nn )
         if (mm /= nn) then
            call gem_xch_halo (wk1(Minx,Miny,HLT_start),Minx,Maxx,Miny,Maxy,local_np,-1)
         end if
