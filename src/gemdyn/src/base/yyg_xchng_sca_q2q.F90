...
!----------------------------------------------------------------------
!
      call gem_xch_halo_st ( F_src, Minx,Maxx,Miny,Maxy, NK, -1)
...
! Fill my results buffers if I have received something
      ...
      if (do_xch) then
         call gem_xch_halo_st ( F_src, l_minx,l_maxx,l_miny,l_maxy,Nk,-1)
      endif
!
!----------------------------------------------------------------------
!
      ...
      subroutine yyg_xchng_sca_q2q_hlt ( F_src, F_comm, Minx,Maxx,Miny,Maxy,&
                                     F_ni, F_nj, Nk, F_interpo_S       ,&
                                     mono_L, do_xch )
      use ISO_C_BINDING
      use yyg_param
...
!----------------------------------------------------------------------
!
      call yyg_SendRecv_s_hlt ( F_src, F_comm, Minx,Maxx,Miny,Maxy, &
                                NK, F_interpo_S, mono_L )
...
!!$omp enddo
      ...
      if (do_xch) then
         ...
         call gem_xch_halo ( F_src(l_minx,l_miny,HLT_start),&
...
!
      return
      end subroutine yyg_xchng_sca_q2q_hlt
