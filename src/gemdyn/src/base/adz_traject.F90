...
!---------------------------------- LICENCE END ---------------------------------
      ...
      subroutine adz_traject (F_dt_8, itpc, F_euler_L, F_dep_L)
      use ISO_C_BINDING
      use cstv
      use geomh
      use gmm_vt0
      ...
      logical, intent(IN) :: F_euler_L,F_dep_L
...
!!$omp end do
      ...
      do iter = 1, Schm_itSL
         ...
         do k= k00, l_nk
            do j= 1, l_nj
               do i= 1, l_ni
                  xm(i) = dble(i+l_i0-1) - tau*(Adz_uvw_dep(1,i,j,k) +&
                                   Adz_uu_arr(i,j,k))*geomh_inv_hx_8
                  ym(i) = dble(j+l_j0-1) - tau*(Adz_uvw_dep(2,i,j,k) +&
                                   Adz_vv_arr(i,j,k))*geomh_inv_hy_8
                  pos   = Ver_z_8%m(k)   - tau*(Adz_uvw_dep(3,i,j,k) +&
                                    Adz_ww_arr(i,j,k))
         ...
         !2. interpolate V(t) to latest midpoint x^0 estimate
         do k= k00, l_nk
         call tricublin_zyx3_n ( Adz_uvw_dep(1,1,1,k),Adz_uvw_d(1,1,1,1), &
                                 Adz_pxyzm(1:3,:,:,k),Adz_cpntr_q,Adz_2dnh)
         end do
         ...
         if (F_dep_L  ) then
            ...
            if (F_euler_L) then
               ...
               !for the backward Euler step, set fake time level x^- = x^0 
               do k= k00, l_nk
                  do j= 1, l_nj
                     do i= 1, l_ni
      ...
      end do !end Schm_itSL

!---copied over from SW_adz_traj---
!!$omp do
      do k= Adz_k0, l_nk
