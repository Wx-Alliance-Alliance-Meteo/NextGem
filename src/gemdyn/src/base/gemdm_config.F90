...
#include <rmnlib_basics.hf>
      ...
      integer :: i, ipcode, ipkind, nsteps, err
...
!!$      end if
      ...
      !     Some common setups for Shallow-water runs
...
!!$      end if
      ...
      if ( Vtopo_start_S  == '' ) then
         Vtopo_start_8= dble(Step_initial) * Step_dt
      else
         err= min( timestr2step (nsteps,Vtopo_start_S,Step_dt),err)
         Vtopo_start_8= dble(nsteps) * Step_dt
      end if
      if ( Vtopo_length_S  == '' ) then
         Vtopo_ndt_8= 0.
      else
         err= min( timestr2step (nsteps,Vtopo_length_S,Step_dt),err)
         Vtopo_ndt_8= dble(nsteps) * Step_dt
      end if
      Vtopo_L = (Vtopo_ndt_8 > 0)
