...
!
      ...
      subroutine yyg_SendRecv_s_hlt ( F_src, F_comm, Minx,Maxx,Miny,Maxy, &
                                      NK, F_interpo_S, mono_L)
      use ISO_C_BINDING
      use, intrinsic :: iso_fortran_env
      implicit none
...
!
      ...
      call gem_xch_halo ( F_src(Minx,Miny,HLT_start),&
...
!
      ...
      integer :: HLT_np, HLT_start, HLT_end
      integer tag1, tag2, ireq, kk, m, mm, kk_proc, adr, ind1, ind2
      integer ierr,status(mpi_status_size,Ptopo_numproc*4)
...
! Posting the receive requests
      do kk= 1, F_commU%recvmaxproc
         ...
         if (m > 0) then
            ireq = ireq+1
            call MPI_IRecv (YYG_urecvuv(1,KK), m*NK         ,&
      ...
      do kk= 1, F_commV%recvmaxproc
         ...
         if (m > 0) then
            ireq = ireq+1
            call MPI_IRecv (YYG_vrecvuv(1,KK), m*NK         ,&
                            MPI_REAL, kk_proc,tag2+kk_proc,&
                            COMM_multigrid,request(ireq),ierr)
         end if
      end do
...
! Interpolation and shipping
      do kk= 1, F_commU%sendmaxproc
         ...
         if (mm > 0) then
             ...
             ireq = ireq+1
             call MPI_ISend ( YYG_usenduv(1,KK), mm*NK, MPI_REAL,&
      ...
      do kk= 1, F_commV%sendmaxproc
         ...
         if (mm > 0) then
             ...
             ireq = ireq+1
             call MPI_ISend ( YYG_vsenduv(1,KK), mm*NK, MPI_REAL,&
                              kk_proc, tag2+Ptopo_world_myproc,&
                              COMM_multigrid, request(ireq), ierr )
         end if
