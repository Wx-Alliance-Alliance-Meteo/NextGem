...
!
      ...
      real, dimension(:,:,:), pointer, intent(INOUT) :: f2rc
      real f2cc(lminx:lmaxx,lminy:lmaxy,Nk)

      integer i, j, k, iproc, tag, err, ni, nj
...
! Receive local data (LD) segments from other processors of bloc
         ...
         do iproc = 1, Bloc_npes-1
            ...
            call MPI_recv ( buf, len, MPI_REAL, Bloc_peid(iproc),&
...
! Send local data (LD) segment to processor 0 of mybloc
         ...
         call MPI_send ( buf, len, MPI_REAL, Bloc_pe0id(Bloc_mybloc),&
...
! Receiving
         if (blk_dist(1,Bloc_mybloc) <= Nk) then
            ...
            allocate (glb(G_ni,G_nj,nz)) ; glb=0.
