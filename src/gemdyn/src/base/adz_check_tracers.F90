...
!**   s/r adz_check_tracers - Initialize Variables for
!      Mass Conservation/Shape-Preservation (not in NAMELIST)
      ...
      include 'mpif.h'
      integer :: n,err,i,j,g_i0,g_in,g_j0,g_jn,i0_sb,&
                 in_sb,j0_sb,jn_sb,comm,flux_keep
...
!
      if (Adz_BC_LAM_flux<0.or.Adz_BC_LAM_flux>2) &
         call gem_error(-1,'ADZ_CHECK_TRACERS','BC_LAM_FLUX not valid-1')
      ...
      do n=1,Tr3d_ntr
         ...
         if (Tr3d_intp(n)/='NONE'.and.Tr3d_intp(n)/='TRICUB'.and.Tr3d_intp(n)/='BICUBH_QV') &
            call gem_error(-1,'ADZ_CHECK_TRACERS','INTP not valid')
      ...
      if (BC_LAM_L.and.Adz_BC_LAM_flux==0) & 
         call gem_error(-1,'ADZ_CHECK_TRACERS','BC_LAM_FLUX not valid-2')
      ...
      call adz_BC_LAM_setup (Adz_BC_LAM_mask_o,Adz_BC_LAM_mask_i,     &
                             Adz_lminx,Adz_lmaxx,Adz_lminy,Adz_lmaxy,l_nk)
