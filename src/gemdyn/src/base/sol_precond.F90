...
!**s/r  sol_precond - Elliptic solver preconditioner

      subroutine sol_precond ( Sols,Rhs_b,Ni,Nj,Nk )
      use glb_ld
      use opr
      use sol_mem
      use prec
...
!
      ...
      do k=1,Nk
         ...
         do j = Nj-1, 1, -1
            do i=1 , Ni
               fdg(i,j,k)=(fdg(i,j,k)-Prec_ci_8(i,j,k)*fdg(i,j+1,k))/&
                           Prec_bi_8(i,j,k)
            end do
         end do
