...
!
      ...
      !-----------------------------
         ...
         if ((istat1<=0).and.Dcmip_case==14) then
             call handle_error(-1,'DCMIP_INIT','Inappropriate list of tracers')
         endif

     !    call dcmip_tracers14_transport (ut1,vt1,wt1,zdt1,tt1,qv,fis0,st1,ps,q1,&
     !                                    l_minx,l_maxx,l_miny,l_maxy,G_nk,.true.)
      ...
      call gem_xch_halo (fis0, l_minx, l_maxx, l_miny, l_maxy, 1, -1)
      ...
      !------------------------------------------------------------------------------------------
         ...
         do k=1,G_nk
            zmom_8(:,:,k) = Ver_z_8%m(k) + (Ver_b_8%m(k)*fis0(:,:) + Ver_c_8%m(k)*orols(:,:))/grav_8
         end do
