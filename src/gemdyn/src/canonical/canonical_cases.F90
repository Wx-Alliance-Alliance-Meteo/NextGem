...
#define SET_GMMUSR_FLAG(MYMETA,MYFLAG) gmm_metadata(MYMETA%l,gmm_attributes(MYMETA%a%key,ior(MYMETA%a%uuid1,MYFLAG),MYMETA%a%uuid2,MYMETA%a%initmode,MYMETA%a%flags))
      ...
      character(len=1024) :: string, action_S
...
!$omp single
 ...
 66   n=index(string,",")
      ...
      if ((.not. Ctrl_testcases_L) .or. (len(trim(action_S))==0)) goto 999
      ...
      !Print dcmip_HEIGHTS
      !-------------------
      if (action_S=="SET_ZETA") then
         if (Dcmip_case==0) goto 999
...
!!$            end if
         ...
         call gem_error(-1,'CANONICAL_CASES','action_S unknown')
