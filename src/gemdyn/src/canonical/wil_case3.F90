...
!**s/r wil_case2 - To setup Williamson Case 2: Steady state nonlinear geostrophic flow (HEIGHT)
      ...
      subroutine wil_case3 (F_gz,F_oro,F_minx,F_maxx,F_miny,F_maxy,F_nk)
      ...
      real,    intent(out) :: F_gz(F_minx:F_maxx,F_miny:F_maxy,F_nk),F_oro(F_minx:F_maxx,F_miny:F_maxy)
      ...
      if (Ptopo_couleur==0) then
         ...
         do j=g_j0,g_jn
            ...
            do i=g_i0,g_in
               ...
               picll(i,j) = -0.5d0*(ubar_8*(-sina_8*cost_8*cosl_8 + cosa_8*sint_8)+ rayt_8*OMEG_8*sint_8)**2 &
                            +0.5d0*(rayt_8*OMEG_8*sint_8)**2 + k1_8
               mount(i,j) = 0.5d0*(rayt_8*OMEG_8*sint_8)**2  + k2_8 
               picll(i,j) = picll(i,j)/grav_8
               mount(i,j) = mount(i,j)/grav_8
